function [timedownsample, Y_dF_all, block_end] = func_Photometry_Signal_Check_v3(blockpath, behavFiles, whichStreams, whichTTL);
...
% BLOCKPATH = '\\niaaa-ebb.niaaa.nih.gov\SBSGTEST\165\165-201229-152738'; 

% BLOCKPATH = strcat(root,'\MATLAB\TDTbin2mat\Photometry\165\165-201218-153602'); 
...
[BehavData,ABETfile,Descriptives, block_end, largeRewSide, smallRewSide]=ABET2TableFn_Chamber_A_v6(behavFiles,[]);
...
idx_end = time2 <= (BehavData.choiceTime(end,:)+30);
...
minLength_array = floor(min([length(GCamp_DC), length(UV_DC)])/10);
GCamp_DC = GCamp_DC(1:min([length(GCamp_DC), length(UV_DC)]));
UV_DC = UV_DC(1:min([length(GCamp_DC), length(UV_DC)]));

time = time(1:min([length(GCamp_DC), length(UV_DC)]));
