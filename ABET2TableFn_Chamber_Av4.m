function [data, ABETdata, Descriptives, block_end] = ABET2TableFn_Chamber_Av4(filename, dummy)
...
Headers={'Trial','Block','ForceFree','bigSmall','stTime','choiceTime'...
    'collectionTime','shock','omission','omissionALL','WL','WSLScode','win_stay','lose_shift','lose_omit','smallRew','bigRew'};
data=table(zeros(80,1),zeros(80,1),zeros(80,1),zeros(80,1),zeros(80,1),zeros(80,1),zeros(80,1),zeros(80,1),zeros(80,1),zeros(80,1),zeros(80,1),zeros(80,1),zeros(80,1),zeros(80,1),zeros(80,1),zeros(80,1),zeros(80,1));
data.Properties.VariableNames([1:17])=Headers;
...
for jj=1: numel(data.Trial)
    ...
    if data.omission(jj)==0
        if data.bigSmall(jj)==1.2 && data.ForceFree(jj)==0
            data.bigRew(jj)=1;
            if data.shock(jj)==0
                data.WL(jj)=1; %win
        ...
        if data.bigSmall(jj)==0.3 && data.ForceFree(jj)==0
                   data.smallRew(jj)=1;       
        end
        if jj>1
            if data.WL(jj-1)==1
